...
def train(
    ...
    if skip_training_and_use_pretrained_hf_lora_url is not None:
        ...
        training_output_path = TrainingOutput(weights=Path(output_path))
        print("Uploading file to s3")
        file_name = os.path.basename(training_output_path)
        print("file name of safetensors file - ", file_name)
        s3.upload_file(training_output_path, s3_bucket_models, file_name)
